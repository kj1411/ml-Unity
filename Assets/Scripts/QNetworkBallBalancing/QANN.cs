...
public class QANN{
	...
	public double alpha;
	List<QLayer> layers = new List<QLayer>();

	public QANN(int nI, int nO, int nH, int nPH, double a)
	{
		numInputs = nI;
		numOutputs = nO;
		numHidden = nH;
		...
		if(numHidden > 0)
		{
			layers.Add(new QLayer(numNPerHidden, numInputs));

			for(int i = 0; i < numHidden-1; i++)
			{
				layers.Add(new QLayer(numNPerHidden, numNPerHidden));
			}

			layers.Add(new QLayer(numOutputs, numNPerHidden));
		}
		else
		{
			layers.Add(new QLayer(numOutputs, numInputs));
		}
	}
	...
	{
		string weightStr = "";
		foreach(QLayer l in layers)
		{
			foreach(QNeuron n in l.neurons)
			{
