...
public class PongANN{
	...
	public double alpha;
	List<PongLayer> layers = new List<PongLayer>();

	public PongANN(int nI, int nO, int nH, int nPH, double a)
	{
		numInputs = nI;
		numOutputs = nO;
		numHidden = nH;
		...
		if(numHidden > 0)
		{
			layers.Add(new PongLayer(numNPerHidden, numInputs));

			for(int i = 0; i < numHidden-1; i++)
			{
				layers.Add(new PongLayer(numNPerHidden, numNPerHidden));
			}

			layers.Add(new PongLayer(numOutputs, numNPerHidden));
		}
		else
		{
			layers.Add(new PongLayer(numOutputs, numInputs));
		}
	}
	...
	{
		string weightStr = "";
		foreach(PongLayer l in layers)
		{
			foreach(PongNeuron n in l.neurons)
			{
