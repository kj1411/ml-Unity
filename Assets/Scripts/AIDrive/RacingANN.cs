...
public class RacingANN{
	...
	public double alpha;
	List<RacingLayer> layers = new List<RacingLayer>();

	public RacingANN(int nI, int nO, int nH, int nPH, double a)
	{
		numInputs = nI;
		numOutputs = nO;
		numHidden = nH;
		...
		if(numHidden > 0)
		{
			layers.Add(new RacingLayer(numNPerHidden, numInputs));

			for(int i = 0; i < numHidden-1; i++)
			{
				layers.Add(new RacingLayer(numNPerHidden, numNPerHidden));
			}

			layers.Add(new RacingLayer(numOutputs, numNPerHidden));
		}
		else
		{
			layers.Add(new RacingLayer(numOutputs, numInputs));
		}
	}
	...
	{
		string weightStr = "";
		foreach(RacingLayer l in layers)
		{
			foreach(RacingNeuron n in l.neurons)
			{
